...
cd ..
log using "results/fig_2c", replace text
...
*Define variables
local NPW "npw_*01 npw_*03 npw_*04 npw_*05 npw_*06 npw_*07 npw_*08 npw_*09 npw_*02"
local YGN "ygn_*01 ygn_*03 ygn_*04 ygn_*05 ygn_*06 ygn_*07 ygn_*08 ygn_*09 ygn_*02"
local X "i.year i.township_id i.year##c.log_pop_02 i.year##c.area i.year##c.x i.year##c.y"
...
* Limit the x-axis from 2001 to 2009
local xmin = 2001  //Minimum year relative to the intervention (2001)
local xmax = 2009   //Maximum year relative to the intervention (2009)
...
gen coef_new =.
replace coef_new = 0 if time_event == 2002
gen coef_old =.
replace coef_old = 0 if time_event == 2002
...
xscale(range(`xmin', `xmax')) /// Limit the x-axis from 2001 to 2009
xlabel(2001(1)2009, angle (45) nogrid format(%9.0f)) /// Customize x-axis labels
...
graph export "results/fig_2c.eps", replace
