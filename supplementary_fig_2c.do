...
cd ..
log using "results/supplementary_fig_5c", replace text
...
*Define variables
local NPW "npw_*02 npw_*03 npw_*04 npw_*05 npw_*06 npw_*07 npw_*08 npw_*09 npw_*10 npw_*01"
local YGN "ygn_*02 ygn_*03 ygn_*04 ygn_*05 ygn_*06 ygn_*07 ygn_*08 ygn_*09 ygn_*10 ygn_*01"
local X "i.year i.township_id i.year##c.log_pop_01 i.year##c.area i.year##c.x i.year##c.y"

*NPW the new capital
reghdfe log_aod `NPW' if ygn != 1, a(`X') cl(district_id)
...
*YGN the new capital
reghdfe log_aod `YGN' if npw != 1, a(`X') cl(district_id)
...
* Limit the x-axis from 2001 to 2010
local xmin = 2001  //Minimum year relative to the intervention (2001)
...
twoway (scatter coef_new time_event_new, mcolor(cranberry) msymbol(smcircle) msize(medium)) ///
...
xscale(range(`xmin', `xmax')) /// Limit the x-axis from 2001 to 2010
xlabel(2001(1)2010, angle (45) nogrid format(%9.0f)) /// Customize x-axis labels
xline(2001, lcolor(navy) lpattern(dash)) xtitle("Year") /// Add a vertical dotted line at 2002 and the X-axis title
legend(order(1 "Putrajaya" 4 "Kuala Lumpur") region(lcolor(none)) position(1) col(2)) //

graph export "results/supplementary_fig_5c.eps", replace
